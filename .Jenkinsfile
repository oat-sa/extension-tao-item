    ...
    stages {
        stage('Resolve TAO dependencies') {
            environment {
                ...
                REPO_NAME='oat-sa/extension-tao-item'
            }
            steps {
                sh(
        ...
        stage('Tests') {
            parallel {
                stage('Backend Tests') {
                    when {
                        expression {
                            fileExists('build/taoItems/test/unit')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build'){
                            sh(
                                ...
                                script: './vendor/bin/phpunit taoItems/test/unit'
                            )
                        }
                    }
                }
                stage('Frontend Tests') {
                    when {
                        expression {
                            fileExists('build/taoItems/views/build/grunt/test.js')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build/tao/views/build') {
                            ...
                            sh (
                                ...
                                script: 'npx grunt connect:test taoitemstest'
                            )
                        }
                    }
                }
