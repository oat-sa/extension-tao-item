...
TAO_MATCHING.Matching.prototype={checkOptions:function(options){if(typeof options=='string')options=eval('('+options+')');else if(options==null)options={};return options;},evaluate:function()
...
return returnValue;},setValue:function(data)
{this.value=[];for(var key in data){if(data[key]instanceof TAO_MATCHING.BaseTypeVariable){this.value[key]=data[key];}else{if(TAO_MATCHING.VariableFactory.isValidBaseType(data[key])){this.value[key]=TAO_MATCHING.VariableFactory.create(data[key]);}else{throw new Error('TAO_MATCHING.Tuple::setValue an error occured : types of the element is not allowed');}}}}};TAO_MATCHING.Tuple.prototype=$.extend({},TAO_MATCHING.Collection.prototype,TAO_MATCHING.Tuple.prototype);
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.engine=null;function matchingInit(params){var options={"url":null,"params":null,"data":null,"format":"json","options":null};if(typeof(params)!='undefined')$.extend(options,params);if(options.url!=null){TAO_MATCHING.engine=new TAO_MATCHING.MatchingRemote(options.url,options.params,options.options);}
