...
{var returnValue=Array();var options={format:null};if(pOptions!=undefined)$.extend(options,pOptions);switch(options.format){case'JSON':for(var key in this.outcomes){returnValue[key]=[];returnValue[key]["identifier"]=key;returnValue[key]["value"]=this.outcomes[key].toJSon();}
...
{var options={format:null};if(pOptions!=undefined)$.extend(options,pOptions);var returnValue=new Object();switch(options.format){case'JSON':for(var key in this.responses){returnValue[key]=[];returnValue[key]["identifier"]=key;returnValue[key]["value"]=this.responses[key].toJSon();}
break;default:returnValue=this.outcomes;}
return this.responses;},setRule:function(rule)
