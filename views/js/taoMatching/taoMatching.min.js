...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Map=function(data){this.setValue(data);};TAO_MATCHING.Map.prototype={map:function(matchingVar){var returnValue=0.0;var mapKeyFound=[];for(var mapKey in this.value){if(TAO_MATCHING.Variable.isCollection(matchingVar)){for(var varKey in matchingVar.value){if(matchingVar.value[varKey].match(this.value[mapKey]['key'])){returnValue+=this.value[mapKey]['value'];break;}}}
