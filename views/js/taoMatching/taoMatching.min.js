...
throw new Error('TAO_MATCHING.Matching::setMaps a correct variable with the identifier '+data[key].identifier+' exists yet');console.log(data[key].value);var matchingVar=new TAO_MATCHING.Map(data[key]);this.maps[data[key].identifier]=matchingVar;}
...
throw new Error('TAO_MATCHING.Matching::setAreaMaps a correct variable with the identifier '+data[key].identifier+' exists yet');var matchingVar=new TAO_MATCHING.AreaMap(data[key].value);this.areaMaps[data[key].identifier]=matchingVar;}
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Map=function(data){console.log(data);this.value=null;this.lowerBound=null;this.upperBound=null;this.defaultValue=0;this.setValue(data.value);if(typeof data.defaultValue!='undefined'){this.defaultValue=data.defaultValue;}
if(typeof data.lowerBound!='undefined'){this.lowerBound=data.lowerBound;}
if(typeof data.upperBound!='undefined'){this.upperBound=data.upperBound;}};TAO_MATCHING.Map.prototype={map:function(matchingVar){var returnValue=0.0;var mapEntriesFound=new Array();for(var mapKey in this.value){if(TAO_MATCHING.Variable.isCollection(matchingVar)){for(var varKey in matchingVar.value){var found=false;if(matchingVar.value[varKey].match(this.value[mapKey]['key'])){mapEntriesFound.push(varKey);if(!found){returnValue+=this.value[mapKey]['value'];found=true;}}}}
else{if(matchingVar.match(this.value[mapKey]['key'])){mapEntriesFound.push(mapKey);returnValue+=this.value[mapKey]['value'];break;}}}
if(this.defaultValue!=0){var delta=matchingVar.value.length-mapEntriesFound.length;returnValue+=delta*this.defaultValue;}
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.AreaMap=function(data){this.setValue(data);};TAO_MATCHING.AreaMap.prototype={map:function(matchingVar){var returnValue=0.0;var mapKeyFound=[];for(var mapKey in this.value){if(matchingVar instanceof TAO_MATCHING.Tuple){if(this.value[mapKey].key.contains(matchingVar)){returnValue=this.value[mapKey].value;}}
else if(matchingVar instanceof TAO_MATCHING.List){for(var varKey in matchingVar.value){if(this.value[mapKey].key.contains(matchingVar.value[varKey])){returnValue+=this.value[mapKey].value;}}}}
return returnValue;},setValue:function(data){this.value=[];for(var i in data){this.value.push({"value":data[i].value,"key":TAO_MATCHING.VariableFactory.create(data[i].key)});}}};
