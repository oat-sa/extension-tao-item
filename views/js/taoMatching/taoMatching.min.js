...
return funcName+'(';});this.rule=rule;},createVariable:function(options,type){var returnValue=null;options=this.checkOptions(options);if(typeof(options.type)=='undefined'){returnValue=TAO_MATCHING.VariableFactory.create(this.createVariable.arguments[1]);}
else
...
returnValue=parseFloat(Math.min(options.min+(Math.random()*(options.max-options.min)),options.max).toFixed(precision));return returnValue;},contains:function(options,expr1,expr2){var returnValue=null;options=this.checkOptions(options);if(!TAO_MATCHING.Variable.isCollection(expr1)){throw new Error("TtaoItems_models_classes_Matching_Matching::contains \
            an error occured : The operator contains as first argument an expression of type Collection");}
returnValue=expr1.contains(expr2,options);return returnValue;}};
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Ellipse=function(data){TAO_MATCHING.Shape.call(this,data.type);this.center=TAO_MATCHING.VariableFactory.create(data.center);this.hradius=data.hradius;this.vradius=data.vradius;}
...
else if(TAO_MATCHING.Variable.isList(matchingVar)&&needleType=='collection'){returnValue=false;for(var key in matchingVar.value){if(this.contains(matchingVar.value[key]),{"matchingVarList":false}){returnValue=true;}else{returnValue=false;break;}}}
