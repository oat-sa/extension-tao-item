...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Matching=function(pData,pOptions){var data={"outcomes":null,"corrects":null,"maps":null,"rule":null};if(typeof(pData)!='undefined')$.extend(data,pData);var options={"evaluateCallback":null};if(typeof(pOptions)!='undefined')$.extend(options,pOptions);this.corrects=[];this.maps=[];this.outcomes=[];this.responses=[];this.rule='';this.options=options;this.whiteFunctionsList={'and':{'mappedFunction':'andExpression'},'equal':{},'if':{'jsFunction':true},'isNull':{},'getCorrect':{},'getMap':{},'getResponse':{},'mapResponse':{},'match':{},'setOutcomeValue':{},'variable':{'mappedFunction':'getResponse'},'correct':{'mappedFunction':'getCorrect'}};if(data.corrects!=null){this.setCorrects(data.corrects);}
...
TAO_MATCHING.Matching.prototype={evaluate:function()
{console.log(this.getRule());with(this){eval(getRule());}
if(this.options.evaluateCallback!=null)
this.options.evaluateCallback();},outcomesToJSON:function()
...
throw new Error('TAO_MATCHING.Matching::setResponses a correct variable with the identifier '+data[key].identifier+' exists yet');var matchingVar=TAO_MATCHING.VariableFactory.create(data[key].value);this.responses[data[key].identifier]=matchingVar;}
...
return funcName+' (';});this.rule=rule;},setOutcomeValue:function(id,value)
{var outcome=this.getOutcome(id);if(outcome==null)
throw new Exception('TAO_MATCHING.Matching::setOutcomeValue error : the outcome value '+id+' does not exist');outcome.setValue(value);},getCorrect:function(id)
{var returnValue=null;if(typeof(this.corrects[id])!='undefined')
returnValue=this.corrects[id];return returnValue;},getMap:function(id)
{var returnValue=null;if(typeof(this.maps[id])!='undefined')
returnValue=this.maps[id];return returnValue;},getOutcome:function(id)
{var returnValue=null;if(typeof(this.outcomes[id])!='undefined')
returnValue=this.outcomes[id];return returnValue;},getResponse:function(id)
{var returnValue=null;if(typeof(this.responses[id])!='undefined')
returnValue=this.responses[id];return returnValue;},getRule:function()
{return this.rule;},and:function(){var result=true;var paramCount=this.and.arguments.length;for(var i=0;i<paramCount;i++){var subExp=this.and.arguments[i];subExpValue=null;if(subExp instanceof TAO_MATCHING.BaseTypeVariable){if(subExp.Type()!='boolean'){throw new Error('AND operator requires sub-expressions with single cardinality and boolean baseType');}
subExpValue=subExp.getValue();}
else
if(typeof subExp!='boolean'){throw new Error('AND operator requires sub-expressions with single cardinality and boolean baseType');}
else{subExpValue=subExp;}
result=result&&subExpValue;}
return result;},equal:function(subExp1,subExp2){var result=null;var allowedQTITypes=['float','integer','duration'];var allowedBasicTypes=['number'];var value1=null;var value2=null;if(subExp1 instanceof TAO_MATCHING.BaseTypeVariable){if(allowedQTITypes.indexOf(subExp1.getType())<0)
throw new Error('EQUAL operator error : the first argument must be numerical baseType');value1=subExp1.getValue();}
else
if(allowedBasicTypes.indexOf(typeof subExp1)<0){}
else{value1=subExp1;}
if(subExp2 instanceof QTIVariable){if(allowedQTITypes.indexOf(subExp2.getBaseType())<0)
throw new Error('EQUAL operator error : the second argument must be numerical baseType');value2=subExp2.values[0];}
else
if(allowedBasicTypes.indexOf(typeof subExp2)<0){throw new Error('EQUAL operator error : the second argument must be numerical baseType');}
else{value2=subExp2;}
if(value1!=null&&value2!=null)
result=value1==value2;return result;},isNull:function(matchingVar)
{return matchingVar.isNull();},mapResponse:function(map,matchingVar)
{if(!(map instanceof TAO_MATCHING.Map))
throw new Error('TAO_MATCHING.Matching::mapResponse an error occured : first argument expected type TAO_MATCHING.Map, given : '+(typeof map));return map.map(matchingVar);},match:function(expr1,expr2)
{var returnValue=false;if(typeof(expr1)=='undefined')
...
{var self=this;var responses=this.responses;$.ajax({url:this.url,type:'POST',async:true,dataType:'json',data:"data="+JSON.stringify(this.responses)+"&params="+JSON.stringify(self.params),success:function(data){self.outcomes=data;if(self.options.evaluateCallback!=null)
self.options.evaluateCallback(data);}});},outcomesToJSON:function()
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.VariableFactory={create:function(data,pType){var returnValue=null;var varValue=data;var type=null;if(typeof pType=='undefined'||pType==null){if(typeof data=='object'){if(varValue==null){type='NULL';}
else if(($.isArray(data))){type='list';}
else{type='tuple';}}
else{type=typeof data;}}
else{type=pType;}
switch(type){case'tuple':returnValue=new TAO_MATCHING.Tuple(data);break;case'list':returnValue=new TAO_MATCHING.List(data);break;case'boolean':case'number':case'string':case'NULL':returnValue=new TAO_MATCHING.BaseTypeVariable(data);break;default:throw new Error('TAO_MATCHING.VariableFactory::create variable type unknown '+type+' for '+varValue);}
return returnValue;}}
...
{this.value=data;}};TAO_MATCHING.BaseTypeVariable.prototype=$.extend({},TAO_MATCHING.Variable.prototype,TAO_MATCHING.BaseTypeVariable.prototype);
...
{this.value=[];for(var i in data){var matchingVar=TAO_MATCHING.VariableFactory.create(data[i]);this.value.push(matchingVar);}}};TAO_MATCHING.List.prototype=$.extend({},TAO_MATCHING.Collection.prototype,TAO_MATCHING.List.prototype);
...
for(var key in this.value){var compareElt=tuple.getElement(key);if(compareElt==null){returnValue=false;break;}else if(this.value[key].getType()!=compareElt.getType()){throw new Exception('TAO_MATCHING::match an error occured : types of the elements to match are not the same ['+elt.getType()+'] and ['+compareElt.getType()+']');returnValue=false;break;}else if(!this.value[key].match(compareElt)){returnValue=false;break;}else{returnValue=true;}}
return returnValue;},setValue:function(data)
{this.value=[];for(var key in data){var matchingVar=TAO_MATCHING.VariableFactory.create(data[key]);this.value[key]=matchingVar;}}};TAO_MATCHING.Tuple.prototype=$.extend({},TAO_MATCHING.Collection.prototype,TAO_MATCHING.Tuple.prototype);
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.RULE=typeof TAO_MATCHING.RULE!='undefined'?TAO_MATCHING.RULE:{};TAO_MATCHING.RULE.MATCH_CORRECT="if(match(getResponse('RESPONSE'), getCorrect('RESPONSE'))) setOutcomeValue('SCORE', 1); else setOutcomeValue('SCORE', 0);";TAO_MATCHING.RULE.MAP_RESPONSE="if(isNull(getResponse('RESPONSE'))) { setOutcomeValue('SCORE', 0); } else { setOutcomeValue('SCORE', mapResponse(getMap('RESPONSE'), getResponse('RESPONSE'))); }";
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.engine=null;function matching_init(params){var options={"url":null,"params":null,"data":null,"format":"json","options":null};if(typeof(params)!='undefined')$.extend(options,params);if(options.url!=null){TAO_MATCHING.engine=new TAO_MATCHING.MatchingRemote(options.url,options.params,options.options);}
else if(options.data!=null){TAO_MATCHING.engine=new TAO_MATCHING.Matching(options.data,options.options);}
else{throw new Error("matching_init an error occured : the options are not well formed, data or url have to be defined");}}
function matching_evaluate(){TAO_MATCHING.engine.evaluate();}
function matching_getOutcomes(){return TAO_MATCHING.engine.outcomesToJSON();}
function matching_setCorrects(data){TAO_MATCHING.engine.setCorrects(data);}
function matching_setMaps(data){TAO_MATCHING.engine.setMaps(data);}
function matching_setOutcomes(data){TAO_MATCHING.engine.setOutcomes(data);}
function matching_setResponses(data){TAO_MATCHING.engine.setResponses(data);}
function matching_setRule(rule){TAO_MATCHING.engine.setRule(rule);}