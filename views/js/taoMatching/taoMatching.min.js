...
return funcName+'(';});this.rule=rule;},and:function(options){var returnValue=null;options=this.checkOptions(options);var paramCount=this.and.arguments.length;for(var i=1;i<paramCount;i++){var subExp=this.and.arguments[i];var matchingSubExp=TAO_MATCHING.VariableFactory.toBooleanBaseType(subExp);if(matchingSubExp==null){throw new Error("TtaoItems_models_classes_Matching_Matching::and an error occured : The ["+i+"]expression passed ["+matchingSubExp+"] to the operator has to be a valid boolean expression with single cardinality");}else{if(matchingSubExp.isNull()){returnValue=null;break;}else{if(returnValue===null){returnValue=matchingSubExp.getValue();}else{returnValue=returnValue&&matchingSubExp.getValue();}}}}
...
return returnValue;},contains:function(options,expr1,expr2){var returnValue=null;options=this.checkOptions(options);if(!TAO_MATCHING.Variable.isCollection(expr1)){throw new Error("TtaoItems_models_classes_Matching_Matching::contains \
