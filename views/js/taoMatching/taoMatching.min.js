...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Matching=function(){this.corrects=[];this.maps=[];this.outcomes=[];this.responses=[];this.rule='';this.whiteFunctionsList={'and':{'mappedFunction':'andExpression'},'equal':{},'if':{'jsFunction':true},'isNull':{},'getCorrect':{},'getMap':{},'getResponse':{},'mapResponse':{},'match':{},'setOutcomeValue':{}};}
TAO_MATCHING.Matching.prototype={evaluate:function()
{with(this){eval(getRule());}},getCorrect:function(id)
{var returnValue=null;if(typeof(this.corrects[id])!='undefined')
returnValue=this.corrects[id];return returnValue;},getJSonOutcomes:function()
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.taoMatching=null;function matching_init(params){var options={mode:null,outcomes:[],corrects:[],maps:[],rule:""};$.extend(options,params);if($.inArray(options.mode,["client","server"])<0){throw new Error("matching_init an error occured : the mode is not well defined. Allowed modes [client/server], "+options.mode+" given");}
TAO_MATCHING.taoMatching=new TAO_MATCHING.Matching();if($.trim(options.rule)!=""){matching_setRule($.trim(options.rule));}
if(options.outcomes.length){matching_setOutcomes(options.outcomes);}
if(options.corrects.length){matching_setCorrects(options.corrects);}
if(options.maps.length){matching_setMaps(options.maps);}}
function matching_evaluate(){return TAO_MATCHING.taoMatching.evaluate();}
function matching_getOutcomes(){return TAO_MATCHING.taoMatching.getJSonOutcomes();}
function matching_setCorrects(data){return TAO_MATCHING.taoMatching.setCorrects(data);}
function matching_setMaps(data){return TAO_MATCHING.taoMatching.setMaps(data);}
function matching_setOutcomes(data){return TAO_MATCHING.taoMatching.setOutcomes(data);}
function matching_setResponses(data){return TAO_MATCHING.taoMatching.setResponses(data);}
function matching_setRule(rule){TAO_MATCHING.taoMatching.setRule(rule);}