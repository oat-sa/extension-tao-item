...
return returnValue;},randomInteger:function(options){var returnValue=null;options=this.checkOptions(options);var randomNum=Math.random()*(options.max-options.min);returnValue=Math.round(randomNum)+options.min;return returnValue;}};
...
{if(!$.isArray(data))
throw new Error('TAO_MATCHING.Matching::setResponses is waiting on an array, a '+(typeof data)+' is given');for(var key in data){console.log(data[key])
try{if(typeof this.responses[data[key].identifier]!='undefined')
