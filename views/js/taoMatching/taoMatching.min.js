...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Matching=function(pData,pOptions){var data={"outcomes":null,"corrects":null,"maps":null,"rule":null};if(typeof(pData)!='undefined')$.extend(data,pData);var options={"evaluateCallback":null};if(typeof(pOptions)!='undefined')$.extend(options,pOptions);this.corrects=[];this.maps=[];this.outcomes=[];this.responses=[];this.rule='';this.options=options;this.whiteFunctionsList={'and':{'mappedFunction':'andExpression'},'createVariable':{},'equal':{},'if':{'jsFunction':true},'isNull':{},'getCorrect':{},'getMap':{},'getResponse':{},'mapResponse':{},'match':{},'setOutcomeValue':{}};if(data.corrects!=null){this.setCorrects(data.corrects);}
...
TAO_MATCHING.Matching.prototype={checkOptions:function(options){if(typeof options=='string')options=eval('('+options+')');return options;},evaluate:function()
{with(this){eval(getRule());}
if(this.options.evaluateCallback!=null)
this.options.evaluateCallback(this.outcomes);},getRule:function()
...
{switch(options.type){case'integer':case'float':case'string':case'boolean':returnValue=TAO_MATCHING.VariableFactory.create(this.createVariable.arguments[1]);break;case'tuple':var values=[];var a=0;for(var i=1;i<this.createVariable.arguments.length;++i,++a){values[a]=this.createVariable.arguments[i];if(values[a]instanceof TAO_MATCHING.BaseTypeVariable){console.log('is well a base type variable');}}
returnValue=TAO_MATCHING.VariableFactory.create(values);break;case'list':var values=[];var a=0;for(var i=1;i<this.createVariable.arguments.length;++i,++a){values.array_push(this.createVariable.arguments[i]);}
...
else{if(TAO_MATCHING.VariableFactory.isValidBaseType(value)){outcome.setValue(value);}else{throw new Error('taoItems_models_classes_Matching_Matching::setOutcomeValue error : unable to set a value of this type ['+typeof(value)+']');}}},getCorrect:function(identifier)
...
throw new Error('TAO_MATCHING.Matching::getResponse error : try to reach an unknown outcome variable ['+identifier+']');return returnValue;},and:function(options){var result=true;options=this.checkOptions(options);var paramCount=this.and.arguments.length;for(var i=1;i<paramCount;i++){var subExp=this.and.arguments[i];subExpValue=null;if(subExp instanceof TAO_MATCHING.BaseTypeVariable){if(subExp.Type()!='boolean'){throw new Error('AND operator requires sub-expressions with single cardinality and boolean baseType');}
...
return result;},equal:function(options,subExp1,subExp2){var result=null;options=this.checkOptions(options);var allowedQTITypes=['float','integer','duration'];var allowedBasicTypes=['number'];var value1=null;var value2=null;if(subExp1 instanceof TAO_MATCHING.BaseTypeVariable){if(allowedQTITypes.indexOf(subExp1.getType())<0)
...
result=value1==value2;return result;},isNull:function(options,matchingVar)
...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.VariableFactory={create:function(data,pType){console.log(data);var returnValue=null;var type=null;if(typeof pType=='undefined'||pType==null){if(typeof data=='object'){if(data==null){type='NULL';}
...
return returnValue;},isValidBaseType:function(elt){var returnValue=false;switch(typeof(elt)){case'boolean':case'number':case'string':returnValue=true;}
return returnValue;}}
...
{return this.getValue();}};TAO_MATCHING.BaseTypeVariable.prototype=$.extend({},TAO_MATCHING.Variable.prototype,TAO_MATCHING.BaseTypeVariable.prototype);
...
return returnValue;},setValue:function(data)
{console.log(data);this.value=[];for(var key in data){if(data[key]instanceof TAO_MATCHING.BaseTypeVariable){this.value[key]=data[key];}else{if(TAO_MATCHING.VariableFactory.isValidBaseType(data[key])){this.value[key]=TAO_MATCHING.VariableFactory.create(data[key]);}else{throw new Error('TAO_MATCHING.Tuple::setValue an error occured : types of the element is not allowed');}}}}};TAO_MATCHING.Tuple.prototype=$.extend({},TAO_MATCHING.Collection.prototype,TAO_MATCHING.Tuple.prototype);
