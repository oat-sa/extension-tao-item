...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Matching=function(pData,pOptions){var data={"outcomes":null,"corrects":null,"maps":null,"rule":null,"evaluateCallback":null};if(typeof(pData)!='undefined')$.extend(data,pData);var options={"evaluateCallback":null};if(typeof(pOptions)!='undefined')$.extend(options,pOptions);this.corrects=[];this.maps=[];this.outcomes=[];this.responses=[];this.rule='';this.options=options;this.whiteFunctionsList={'and':{'mappedFunction':'andExpression'},'equal':{},'if':{'jsFunction':true},'isNull':{},'getCorrect':{},'getMap':{},'getResponse':{},'mapResponse':{},'match':{},'setOutcomeValue':{}};if(data.corrects!=null){this.setCorrects(data.corrects);}
...
this.options.evaluateCallback();},getCorrect:function(id)
{var returnValue=null;if(typeof(this.corrects[id])!='undefined')
returnValue=this.corrects[id];return returnValue;},outcomesToJSON:function()
...
return returnValue;},getMap:function(id)
...
{return this.rule;},isNull:function(matchingVar)
...
catch(e){throw e;}}},setOutcomeValue:function(id,value)
{var outcome=this.getOutcome(id);if(outcome==null)
throw new Exception('TAO_MATCHING.Matching::setOutcomeValue error : the outcome value '+id+' does not exist');outcome.setValue(value);},setResponses:function(data)
{if(!$.isArray(data))
...
{var whiteFunctionsList=this.whiteFunctionsList;var self=this;var rule2=rule.replace(/([a-zA-Z_\-1-9]*)[\s]*\(/g,function(str,funcName){if(typeof(whiteFunctionsList[funcName])=='undefined')
throw new Error('TAO_MATCHING.Matching::setRule an error occured, the following expression is unknown '+funcName);else if(typeof self[funcName]=='undefined'&&!(typeof whiteFunctionsList[funcName]['jsFunction']!='undefined'&&whiteFunctionsList[funcName]['jsFunction']))
throw new Error('TAO_MATCHING.Matching::setRule an error occured, the following expression is not yet supported '+funcName);return funcName+' (';});this.rule=rule;}};
