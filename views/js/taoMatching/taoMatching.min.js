...
TAO_MATCHING=typeof TAO_MATCHING!='undefined'?TAO_MATCHING:{};TAO_MATCHING.Matching=function(pData,pOptions){var data={"outcomes":null,"corrects":null,"maps":null,"rule":null};if(typeof(pData)!='undefined')$.extend(data,pData);var options={"evaluateCallback":null};if(typeof(pOptions)!='undefined')$.extend(options,pOptions);this.corrects=[];this.maps=[];this.outcomes=[];this.responses=[];this.rule='';this.options=options;this.whiteFunctionsList={'and':{'mappedFunction':'andExpression'},'createVariable':{},'divide':{},'equal':{},'if':{'jsFunction':true},'isNull':{},'getCorrect':{},'getMap':{},'getResponse':{},'gt':{},'gte':{},'lt':{},'lte':{},'mapResponse':{},'match':{},'product':{},'setOutcomeValue':{},'substract':{},'sum':{}};if(data.corrects!=null){this.setCorrects(data.corrects);}
...
else{if(TAO_MATCHING.BaseTypeVariable.isValidValue(value)){outcome.setValue(value);}else{throw new Error('taoItems_models_classes_Matching_Matching::setOutcomeValue error : unable to set a value of this type ['+typeof(value)+']');}}},getCorrect:function(identifier)
...
{var returnValue=false;matchingExpr1=TAO_MATCHING.VariableFactory.toNumericBaseType(expr1);if(matchingExpr1==null){throw new Error("TAO_MATCHING.Matching::gt an error occured : The first expression passed ["+expr1+"] to the operator has to be a valid numerical expression with single cardinality");}
matchingExpr2=TAO_MATCHING.VariableFactory.toNumericBaseType(expr2);if(matchingExpr2==null){throw new Error("TAO_MATCHING.Matching::gt an error occured : The second expression passed ["+expr2+"] to the operator has to be a valid numerical expression with single cardinality");}
...
return returnValue;},substract:function(options,expr1,expr2){var returnValue=null;matchingExpr1=TAO_MATCHING.VariableFactory.toNumericBaseType(expr1);if(matchingExpr1==null){throw new Error("TAO_MATCHING.Matching::substract an error occured : The first expression passed ["+expr1+"] to the operator has to be a valid numerical expression with single cardinality");}
matchingExpr2=TAO_MATCHING.VariableFactory.toNumericBaseType(expr2);if(matchingExpr2==null){throw new Error("TAO_MATCHING.Matching::substract an error occured : The second expression passed ["+expr2+"] to the operator has to be a valid numerical expression with single cardinality");}
...
return returnValue;},divide:function(options,expr1,expr2){var returnValue=null;matchingExpr1=TAO_MATCHING.VariableFactory.toNumericBaseType(expr1);if(matchingExpr1==null){throw new Error("TAO_MATCHING.Matching::substract an error occured : The first expression passed ["+expr1+"] to the operator has to be a valid numerical expression with single cardinality");}
matchingExpr2=TAO_MATCHING.VariableFactory.toNumericBaseType(expr2);if(matchingExpr2==null){throw new Error("TAO_MATCHING.Matching::substract an error occured : The second expression passed ["+expr2+"] to the operator has to be a valid numerical expression with single cardinality");}
