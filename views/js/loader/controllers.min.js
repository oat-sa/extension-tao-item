...
_initCustomButtons())})}),$(".preview-item-container").append($frame),$("#finishButton").click(function(){itemApi.finish()})})}}};return previewItemRunner}),define("taoItems/controller/routes",[],function(){"use strict";return{Items:{deps:"controller/items/action",actions:{editItem:"controller/items/edit",editItemClass:"controller/items/editItemClass"}},ItemPreview:{actions:{index:"controller/preview/itemRunner"}}}}),define("taoItems/controller/runtime/itemRunner",["jquery","lodash","iframeNotifier","urlParser"],function($,_,iframeNotifier,UrlParser){"use strict";var itemRunner={start:function(options){var $frame=$('<iframe id="item-container" class="toolframe" frameborder="0" style="width:100%;" scrolling="no"></iframe>');
$frame.appendTo("body");var itemId=options.itemId,itemPath=options.itemPath,resultServer=_.defaults(options.resultServer,{module:"taoResultServer/ResultServerApi",params:{}}),itemService=_.defaults(options.itemService,{module:"taoItems/runtime/ItemServiceImpl",params:{}}),clientConfigUrl=options.clientConfigUrl,timeout=options.timeout||30;require([itemService.module,resultServer.module],function(ItemService,ResultServerApi){var resultServerApi=new ResultServerApi(resultServer.endpoint,resultServer.params),setIframeHeight=function(newHeight){var neverResized=150===$frame.height(),resize=function(newHeight,notifyParent){$frame.css({height:newHeight}),notifyParent&&iframeNotifier.parent("heightchange",newHeight);
};resize(newHeight||$frame.contents().outerHeight(!0),!newHeight),neverResized&&_.delay(function(){resize($frame.contents().outerHeight(!0),!0)},200)};window.onServiceApiReady=function(serviceApi){var itemApi=new ItemService(_.merge({serviceApi:serviceApi,itemId:itemId,resultApi:resultServerApi},{params:itemService.params})),itemUrl=new UrlParser(itemPath),isCORSAllowed=itemUrl.checkCORS();itemUrl.addParam("clientConfigUrl",clientConfigUrl),itemUrl.addParam("timeout",timeout),$(document).on("itemloaded",function(){setIframeHeight(),iframeNotifier.parent("serviceloaded"),_.delay(setIframeHeight,1600),$frame.contents().find("img").on("load",_.throttle(function(){
setIframeHeight()},50))}).on("responsechange",function(e,responseId,response){iframeNotifier.parent("responsechange",[responseId,response])}).on("stateready",function(e,elmentId,state){iframeNotifier.parent("stateready",[elmentId,state])}).on("itemready",function(){itemApi.connect($frame[0])}).on("itemcontentchange imageloaded",function(){setIframeHeight()}),$frame.on("load",function(){itemApi.connect($frame[0]),isCORSAllowed===!0&&(this.contentWindow.__knownParent__=!0),$("body").on("setheight",function(event,height){setIframeHeight(height)})}),$frame.attr("src",itemUrl.getUrl())},_.defer(function(){iframeNotifier.parent("serviceready")})})}};return itemRunner;
...
define("taoItems/loader/controllers.min",function(){});
