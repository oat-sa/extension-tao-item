...
$listing.append("<li><pre>"+msgStr+"</pre></li>"),$body.is(":visible")?$listing.scrollTop(1e4):$body.slideDown("slow",function(){$closer.show(),$listing.scrollTop(1e4)})}),$closer.on("click",function(){$body.slideUp("slow",function(){$closer.hide()})})},show=function(){$.ajax({url:itemUri,dataType:"html"}).done(function(data){$(".preview-item-container").html(data)}),overlay[0].style.display="block",overlay.height("100%"),overlay.find("select:visible").not(".preview-theme-selector").trigger("change"),_scale(),_positionPreview()},init=function(_itemUri){if(!_itemUri||0===_itemUri.length)throw new TypeError("Wrong URI");return $(".preview-overlay").remove(),container=null,overlay=$(previewTpl({mobileDevices:_getDeviceSelectorData("mobile"),desktopDevices:_getDeviceSelectorData("desktop"),previewTypes:_getPreviewTypes(),previewType:previewType,themes:_getThemes()})),$body.append(overlay),previewContainerMaxWidth=parseInt($(".preview-container").css("max-width"),10),$feedbackBox=overlay.find(".preview-message-box"),$.cookie("hidePreviewFeedback")&&$feedbackBox.hide(),$console=overlay.find("#preview-console"),_initConsole(),_updateStandardPreviewSize(),_setupTypeDependantElements(),_setupDeviceSelectors(),_setupOrientationSelectors(),_setupViewSelector(),_setupThemeSelector(),_setupClosers(),_computeScaleFactor(),_setPreviewType(previewType),_setOrientation(orientation),$window.on("resize orientationchange",function(){screenSize={width:$window.innerWidth(),height:$window.innerHeight()},_updateStandardPreviewSize(),_computeScaleFactor(),_scale()
