...
})),itemUrl=new UrlParser(itemPath),isCORSAllowed=itemUrl.checkCORS();itemUrl.addParam("clientConfigUrl",clientConfigUrl),itemUrl.addParam("timeout",timeout),$(document).on("itemloaded",function(){iframeNotifier.parent("serviceloaded"),$frame.height($frame.contents().height())}).on("responsechange",function(e,responseId,response){iframeNotifier.parent("responsechange",[responseId,response])}).on("stateready",function(e,elmentId,state){iframeNotifier.parent("stateready",[elmentId,state])}).on("itemready",function(){itemApi.connect($frame[0])}),$(window).on("resize",_.debounce(function(){$frame.height($frame.contents().height())},50)),$frame.on("load",function(){
itemApi.connect($frame[0]),isCORSAllowed===!0&&(this.contentWindow.__knownParent__=!0)}),$frame.attr("src",itemUrl.getUrl())},_.defer(function(){iframeNotifier.parent("serviceready")})})}};return itemRunner});
