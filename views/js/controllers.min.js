...
$listing.append("<li><pre>"+msgStr+"</pre></li>"),$body.is(":visible")?$listing.scrollTop(1e4):$body.slideDown("slow",function(){$closer.show(),$listing.scrollTop(1e4)})}),$closer.on("click",function(){$body.slideUp("slow",function(){$closer.hide()})})},show=function(){$.ajax({url:itemUri,dataType:"html"}).done(function(data){winScrollObj={x:window.scrollX,y:window.scrollY},window.scrollTo(0,0),$body.addClass("preview-mode"),overlay[0].style.display="block",overlay.find("select:visible").not(".preview-theme-selector").trigger("change"),_scale(),_positionPreview(),$(".preview-item-container").html(data)})},init=function(_itemUri){if(!_itemUri||0===_itemUri.length)throw new TypeError("Wrong URI");
return $(".preview-overlay").remove(),container=null,overlay=$(previewTpl({mobileDevices:_getDeviceSelectorData("mobile"),desktopDevices:_getDeviceSelectorData("desktop"),previewTypes:_getPreviewTypes(),previewType:previewType,themes:_getThemes(),hasThemes:_.size(themeObj.available)>1})),$body.append(overlay),previewContainerMaxWidth=parseInt($(".preview-container").css("max-width"),10),$feedbackBox=overlay.find(".preview-message-box"),$.cookie("hidePreviewFeedback")&&($feedbackBox.hide(),$body.addClass("no-preview-feedback")),$console=overlay.find("#preview-console"),_initConsole(),_updateStandardPreviewSize(),_setupTypeDependantElements(),_setupDeviceSelectors(),
_setupOrientationSelectors(),_setupViewSelector(),_setupThemeSelector(),_setupClosers(),_setupScreenSize(),_computeScaleFactor(),_setPreviewType(previewType),_setOrientation(orientation),$window.on("resize orientationchange",function(){_setupScreenSize(),_updateStandardPreviewSize(),_computeScaleFactor(),_scale()}),itemUri=_itemUri,overlay};return{init:init,show:show}}),define("taoItems/controller/items/action",["layout/actions/binder","uri","jquery","context","taoItems/preview/preview","helpers"],function(binder,uri,$,context,preview,helpers){binder.register("itemPreview",function(actionContext){preview.init(helpers._url("forwardMe","ItemPreview",context.shownExtension,{
uri:actionContext.id})),preview.show()})}),define("taoItems/controller/items/edit",["jquery","i18n","module","layout/actions","ui/lock","layout/section"],function($,__,module,actions,lock,section){"use strict";var editItemController={start:function(){var config=module.config(),previewAction=(!!config.isPreviewEnabled,!!config.isAuthoringEnabled,actions.getBy("item-preview")),authoringAction=actions.getBy("item-authoring");previewAction&&(previewAction.state.disabled=!config.isPreviewEnabled),authoringAction&&(authoringAction.state.disabled=!config.isAuthoringEnabled),actions.updateState(),$("#lock-box").each(function(){lock($(this)).register()}),section.off("show").on("show",function(sectionContext){
"manage_items"===sectionContext.id&&actions.exec("item-properties")})}};return editItemController}),define("serviceApi/ServiceApi",["jquery","urlParser"],function($,UrlParser){"use strict";function ServiceApi(baseUrl,parameters,serviceCallId,stateStorage,userService,paused){this.baseUrl=baseUrl,this.parameters=parameters,this.connected=!1,this.serviceCallId=serviceCallId,this.state=stateStorage,this.userService=userService,this.hasBeenPaused=paused}return ServiceApi.SIG_SUCCESS=0,ServiceApi.SIG_ERROR=1,ServiceApi.prototype.loadInto=function(frame,connected){var self=this,$frame=$(frame),callUrl=this.getCallUrl(),isCORSAllowed=new UrlParser(callUrl).checkCORS();
