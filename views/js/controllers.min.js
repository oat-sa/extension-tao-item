...
})}),define("taoItems/preview/preview",["jquery","lodash","i18n","util/strPad","json!taoItems/preview/resources/device-list.json","tpl!taoItems/preview/tpl/preview","ui/themes","ui/modal","select2","jquery.cookie"],function($,_,__,strPad,deviceList,previewTpl,themeHandler){function _setupScreenSize(){screenSize={width:$(".preview-overlay").innerWidth(),height:$(".preview-overlay").innerHeight()}}var overlay,container,typeDependant,$feedbackBox,$console,previewContainerMaxWidth,itemUri,orientation="landscape",previewType="standard",previewTypes={desktop:__("Desktop preview"),mobile:__("Mobile preview"),standard:__("Actual size")},themes=themeHandler.getAvailable("items")||[],$doc=$(document),$window=$(window),$body=$(document.body),screenSize={width:$window.innerWidth(),height:$window.innerHeight()},sizeSettings={width:0,height:0},scaleFactor=1,_getDeviceSelectorData=function(type){var devices="mobile"===type?deviceList.tablets:deviceList.screens,options=[];
...
}),$hideForever.on("click",function(e){e.preventDefault(),$.cookie("hidePreviewFeedback",!0,{expires:1e3,path:"/"}),$feedbackCloser.trigger("click")})},_getThemes=function(){var options=[];return _(themes).forEach(function(data){options.push({value:data.id,label:data.name,selected:!1})}),options},_getPreviewTypes=function(){var options=[];return _(previewTypes).forEach(function(_previewLabel,_previewType){options.push({value:_previewType,label:_previewLabel,selected:previewType===_previewType})}),options},_setupViewSelector=function(){$("select.preview-type-selector").on("change",function(){var _previewType=$(this),value=_previewType.val();_setPreviewType(value),$("."+value+"-device-selector").trigger("change")
}).select2({minimumResultsForSearch:-1})},_setupThemeSelector=function(){$("select.preview-theme-selector").on("change",function(){var iframe=document.getElementById("preview-container"),iframeDoc=iframe.contentWindow||iframe.contentDocument;iframeDoc.$(".qti-item").trigger("themechange",[$(this).val()])}).select2({minimumResultsForSearch:-1})},_adaptFrameSize=function(){var $previewContainer=$(".preview-container"),$iframe=function(){var _iframe=$previewContainer.find("iframe");return _iframe.height(""),_iframe}(),contentHeight=$iframe.contents().outerHeight(),containerHeight=$previewContainer.innerHeight();"standard"!==previewType?containerHeight>contentHeight&&(contentHeight=containerHeight):$previewContainer.height(contentHeight+10),$iframe.height(contentHeight)
