...
setIframeHeight(),iframeNotifier.parent("serviceloaded"),$frame.contents().find("img").on("load",_.throttle(function(){setIframeHeight()},50))}).on("responsechange",function(e,responseId,response){iframeNotifier.parent("responsechange",[responseId,response])}).on("stateready",function(e,elmentId,state){iframeNotifier.parent("stateready",[elmentId,state])}).on("itemready",function(){itemApi.connect($frame[0])}).on("imageloaded",function(){setIframeHeight()}),$frame.on("load",function(){itemApi.connect($frame[0]),isCORSAllowed===!0&&(this.contentWindow.__knownParent__=!0),$("body").on("setheight",function(event,height){setIframeHeight(height)})}),$frame.attr("src",itemUrl.getUrl());
