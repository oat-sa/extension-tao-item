...
}}else message="<br />"+settings.type+" "+settings.url+" ? "+decodeURIComponent(settings.data)+" => "+request.responseText,$previewConsole.trigger("updateConsole",["remote request",message])}})}}}),define("taoItems/controller/preview/itemRunner",["module","jquery","lodash","serviceApi/ServiceApi","serviceApi/PseudoStorage","serviceApi/UserInfoService","taoItems/runtime/ItemServiceImpl","taoItems/preview/preview-console","urlParser","iframeResizer"],function(module,$,_,ServiceApi,PseudoStorage,UserInfoService,ItemServiceImpl,previewConsole,UrlParser,iframeResizer){var previewItemRunner={start:function(options){var conf=_.merge(module.config(),options||{});if(conf.previewUrl){previewConsole.setup();
var resultServer=_.defaults(conf.resultServer,{module:"taoResultServer/ResultServerApi",endpoint:"",params:{}});require([resultServer.module],function(ResultServerApi){var resultServerApi=new ResultServerApi(resultServer.endpoint,resultServer.params),serviceApi=new ServiceApi(conf.previewUrl,{},"preview",new PseudoStorage,new UserInfoService(conf.userInfoServiceRequestUrl,{})),itemApi=new ItemServiceImpl({serviceApi:serviceApi,resultApi:resultServerApi}),callUrl=new UrlParser(serviceApi.getCallUrl()),isCORSAllowed=callUrl.checkCORS();callUrl.addParam("clientConfigUrl",conf.clientConfigUrl);var $frame=$("#preview-container");iframeResizer.autoHeight($frame,"body",10),$frame.on("load",function(){var frame=this;
itemApi.connect(frame),isCORSAllowed===!0&&(frame.contentWindow.__knownParent__=!0),$(document).on("itemready",function(){itemApi.connect(frame)})}),$("#preview-container").attr("src",callUrl.getUrl()),$("#finishButton").click(function(){itemApi.finish()})})}}};return previewItemRunner}),define("taoItems/controller/runtime/itemRunner",["jquery","lodash","iframeResizer","iframeNotifier","urlParser"],function($,_,iframeResizer,iframeNotifier,UrlParser){var itemRunner={start:function(options){var $frame=$('<iframe id="item-container" class="toolframe" frameborder="0" style="width:100%;height:100%;overflow:hidden" scrolling="no"></iframe>');$frame.appendTo("body");
