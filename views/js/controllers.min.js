...
},ServiceApi.prototype.onFinish=function(callback){this.onFinishCallback=callback},ServiceApi.prototype.onKill=function(callback){this.onKillCallback=callback},ServiceApi.prototype.kill=function(callback){"function"==typeof this.onKillCallback?this.onKillCallback(callback):callback(0)},ServiceApi.prototype.finish=function(valueArray){"function"==typeof this.onFinishCallback&&this.onFinishCallback(valueArray)},ServiceApi}),define("serviceApi/PseudoStorage",[],function(){function PseudoStorage(){}return PseudoStorage.prototype.get=function(callback){return"function"==typeof callback&&callback(null),null},PseudoStorage.prototype.set=function(state,callback){"function"==typeof callback&&callback();
},PseudoStorage}),define("serviceApi/UserInfoService",["jquery"],function($){function UserInfoService(requestUrl,data){this.data=data,this.requestUrl=requestUrl}return UserInfoService.prototype.get=function(property,callback){this.data.hasOwnProperty(property)?"function"==typeof callback&&callback(this.data[property]):$.ajax({url:this.requestUrl,data:{property:property},type:"post",dataType:"json",success:function(service,callback){return function(r){for(key in r.data)service.data[key]=r.data[key];"function"==typeof callback&&callback(service.data[property])}}(this,callback)})},UserInfoService}),define("taoItems/runtime/ItemServiceImpl",["jquery"],function($){
function ItemServiceImpl(data){this.itemId=data.itemId,this.serviceApi=data.serviceApi,this.resultApi=data.resultApi||null,this.params=data.params||{},this.responses={},this.scores={},this.events={},this.connected=!1;var rawstate=this.serviceApi.getState(),state="undefined"==typeof rawstate||null===rawstate?{}:$.parseJSON(rawstate);this.stateVariables="object"==typeof state?state:{},this.beforeFinishCallbacks=[]}return ItemServiceImpl.prototype.connect=function(frame){this.connected===!1&&frame.contentWindow&&(frame.contentWindow.itemApi=this,"function"==typeof frame.contentWindow.onItemApiReady&&(frame.contentWindow.onItemApiReady(this),this.connected=!0))},
ItemServiceImpl.prototype.saveResponses=function(valueArray){for(var attrname in valueArray)this.responses[attrname]=valueArray[attrname]},ItemServiceImpl.prototype.traceEvents=function(eventArray){for(var attrname in eventArray)this.events[attrname]=eventArray[attrname]},ItemServiceImpl.prototype.saveScores=function(valueArray){for(var attrname in valueArray)this.scores[attrname]=valueArray[attrname]},ItemServiceImpl.prototype.getUserPropertyValues=function(property,callback){this.serviceApi.getUserPropertyValues(property,callback)},ItemServiceImpl.prototype.beforeFinish=function(callback){this.beforeFinishCallbacks.push(callback)},ItemServiceImpl.prototype.finish=function(){
for(var self=this,i=0;i<this.beforeFinishCallbacks.length;i++)this.beforeFinishCallbacks[i]();this.submit(function(){self.serviceApi.finish()})},ItemServiceImpl.prototype.submit=function(after){var self=this;this.serviceApi.setState(JSON.stringify(self.stateVariables),function(){self.resultApi?self.resultApi.submitItemVariables(self.itemId,self.serviceApi.getServiceCallId(),self.responses,self.scores,self.events,self.params,after):after()})},ItemServiceImpl.prototype.onKill=function(callback){this.serviceApi.onKill(callback)},ItemServiceImpl.prototype.getVariable=function(identifier,callback){"function"==typeof callback&&callback(this.stateVariables[identifier]||null);
...
if(data.data){try{var matchingDataList=$.parseJSON(data.data);for(key in matchingDataList)message+="<br />"+matchingDataList[key].identifier+" = "+matchingDataList[key].value}catch(exp){}message+="<br />",$previewConsole.trigger("updateConsole",["sent answers",message])}}else message="<br />"+settings.type+" "+settings.url+" ? "+decodeURIComponent(settings.data)+" => "+request.responseText,$previewConsole.trigger("updateConsole",["remote request",message])}})}}}),define("taoItems/preview/actionBarHook",["jquery","lodash","core/errorHandler","core/promise"],function($,_,errorHandler,Promise){"use strict";function isValidConfig(toolconfig){return!(!_.isObject(toolconfig)||!toolconfig.hook);
}function triggerItemLoaded(tool){tool&&tool.itemLoaded&&tool.itemLoaded()}function initExtraButtons($toolContainer,id,toolconfig){return _.isString(toolconfig)&&(toolconfig={hook:toolconfig}),tools[id]=null,$toolContainers=$toolContainers.add($toolContainer),new Promise(function(resolve){isValidConfig(toolconfig)?require([toolconfig.hook],function(hook){var $button,$existingBtn;isValidHook(hook)?(hook.init(id,toolconfig),$existingBtn=$toolContainer.children('[data-control="'+id+'"]'),$existingBtn.length&&(hook.clear($existingBtn),$existingBtn.remove()),tools[id]=hook,$button=hook.render(),_appendInOrder($toolContainer,$button),$button.trigger("ready"+_ns,[hook]),
triggerItemLoaded(hook),resolve(hook)):(errorHandler["throw"](_ns,"invalid hook format"),resolve(null))},function(e){errorHandler["throw"](_ns,"the hook amd module cannot be found"),resolve(null)}):(errorHandler["throw"](_ns,"invalid tool config format"),resolve(null))})}function _appendInOrder($container,$button){var $after,$before,order=$button.data("order");"last"===order?$container.append($button):"first"===order?$container.prepend($button):(order=_.parseInt(order),_.isNaN(order)?$container.append($button):($container.children(".action").each(function(){var $btn=$(this),_order=$btn.data("order");if("last"===_order)$before=$btn,$after=null;else if("first"===_order)$before=null,
$after=$btn;else{if(_order=_.parseInt(_order),_.isNaN(_order)||_order>order)return $before=$btn,$after=null,!1;_order===order?$after=$btn:order>_order&&($after=$btn,$before=null)}}),$after?$after.after($button):$before?$before.before($button):$container.append($button)))}function isValidHook(hook){return _.isObject(hook)&&_(["init","render","clear","isVisible"]).reduce(function(result,method){return result&&_.isFunction(hook[method])},!0)}var _ns=".previewActionBarHook",tools={},$toolContainers=$();return $(document).off(".preview").on("itemunload.preview",function(){$toolContainers.each(function(){var $toolContainer=$(this);_.forEach(tools,function(tool,id){
var $btn=$toolContainer.children('[data-control="'+id+'"]');$btn.length&&(tool.clear($btn),$btn.remove())})})}),{isValid:isValidConfig,initExtraButtons:initExtraButtons}}),define("taoItems/controller/preview/itemRunner",["module","jquery","lodash","serviceApi/ServiceApi","serviceApi/PseudoStorage","serviceApi/UserInfoService","taoItems/runtime/ItemServiceImpl","taoItems/preview/preview-console","taoItems/preview/actionBarHook","urlParser"],function(module,$,_,ServiceApi,PseudoStorage,UserInfoService,ItemServiceImpl,previewConsole,actionBarHook,UrlParser){"use strict";var _initCustomButtons=function(){var config=module.config(),buttons=config.extraButtons||{},$container=$(".extra-button-action-bar");
