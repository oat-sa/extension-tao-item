...
_computeScaleFactor(),_scale()}),itemUri=_itemUri,overlay};return{init:init,show:show}}),define("taoItems/controller/items/action",["layout/actions/binder","uri","jquery","context","taoItems/preview/preview","helpers"],function(binder,uri,$,context,preview,helpers){binder.register("itemPreview",function(actionContext){preview.init(helpers._url("forwardMe","ItemPreview",context.shownExtension,{uri:actionContext.id})),preview.show()})}),define("taoItems/controller/items/edit",["jquery","i18n","module","layout/actions","ui/lock","layout/section"],function($,__,module,actions,lock,section){"use strict";var editItemController={start:function(){var config=module.config(),previewAction=(!!config.isPreviewEnabled,
!!config.isAuthoringEnabled,actions.getBy("item-preview")),authoringAction=actions.getBy("item-authoring");previewAction&&(previewAction.state.disabled=!config.isPreviewEnabled),authoringAction&&(authoringAction.state.disabled=!config.isAuthoringEnabled),actions.updateState(),$("#lock-box").each(function(){lock($(this)).register()}),section.off("show").on("show",function(sectionContext){"manage_items"===sectionContext.id&&actions.exec("item-properties")})}};return editItemController}),define("serviceApi/ServiceApi",["jquery","urlParser"],function($,UrlParser){"use strict";function ServiceApi(baseUrl,parameters,serviceCallId,stateStorage,userService){this.baseUrl=baseUrl,
...
itemApi.connect($frame[0])}).on("imageloaded",function(){setIframeHeight()}),$frame.on("load",function(){itemApi.connect($frame[0]),isCORSAllowed===!0&&(this.contentWindow.__knownParent__=!0),$("body").on("setheight",function(event,height){setIframeHeight(height)})}),$frame.attr("src",itemUrl.getUrl())},_.defer(function(){iframeNotifier.parent("serviceready")})})}};return itemRunner});
