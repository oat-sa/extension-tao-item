...
if(this.dataSource.settings.load=='onInit'){this.loadData(source);}}};this.loadData=function(source){var populateData=function(data,instance){if($.isPlainObject(data)){for(key in data){isTaoVar=false;for(uriKey in URI){if(URI[uriKey]==key){instance.taoVars[key]=data[key];isTaoVar=true;break;}}
if(!isTaoVar){instance.dataStore[key]=data[key];}}}};if(this.dataSource.environment.type=='manual'&&source!=null){populateData(source,this);}
else{var params=this.dataSource.environment.params;var instance=this;$.ajax({'url':this.dataSource.environment.url,'data':params,'type':this.dataSource.settings.method,'async':(this.dataSource.environment.type=='async'),'dataType':this.dataSource.settings.format,'success':function(data){if(data.token){populateData(data,instance);}}});}};this.push=new Object();this.push.environment={'url':'/tao/Api/save','params':{'token':this.dataStore.token}};this.push.settings={'format':'json','method':'post','async':true,'clearAfter':true};this.initPush=function(environment,settings){if(environment.url){if(/(\/[-a-z\d%_.~+]*)*(\?[;&a-z\d%_.~+=-]*)?(\#[-a-z\d_]*)?$/.test(environment.url)){this.push.environment.url=environment.url;}}
if($.isPlainObject(environment.params)){for(key in environment.params){if(isScalar(environment.params[key])&&!this.push.environment.params[key]){this.push.environment.params[key]=environment.params[key];}}}
...
if(settings.clearAfter===false){this.push.settings.clearAfter=false;}}};this.push=function(){var params=this.push.environment.params;for(key in this.taoVars){if(/^##NAMESPACE#/.test(key)&&this.dataStore.localNamespace!=undefined){key=key.replace('##NAMESPACE#',this.dataStore.localNamespace);}
params['taoVars'][key]=this.taoVars[key];}
var instance=this;$.ajax({'url':this.push.environment.url,'data':params,'type':this.push.settings.method,'async':this.push.settings.async,'dataType':this.push.settings.format,'success':function(data){if(data.saved){if(instance.push.settings.clearAfter){instance.taoVars=new Object();instance.userVars=new Object();}}}});};this.taoVars=new Object();this.getTaoVar=function(key,label){var value=(this.taoVars[key])?this.taoVars[key]:false;if($.isPlainObject(value)){if((value['uri']!=undefined&&value[URI.LABEL]!=undefined&&value.length==2)||label){return value[URI.LABEL];}}
return value;};this.setTaoVar=function(key,value,property){if(isScalar(value)){var currentValue=(this.taoVars[key])?this.taoVars[key]:false;if($.isPlainObject(currentValue)){if(property){this.taoVars[key][property]=value;}
...
function getSubjectName(){var subject=getSubject();return(subject)?subject[URI.SUBJETC_FIRSTNAME]+' '+subject[URI.SUBJETC_LASTNAME]:'';}
