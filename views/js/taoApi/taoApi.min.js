...
function TaoStack(){this.dataSource=new Object();this.dataSource.environment={'type':'async','url':'/tao/Api/getContext','params':{}};this.dataSource.settings={'format':'json','method':'post','load':'onInit'};this.dataStore=new Object();this.initDataSource=function(environment,settings,source){if($.inArray(environment.type,['manual','sync','async'])){this.dataSource.environment.type=environment.type;if(this.dataSource.environment.type!='manual'&&environment.url){if(/(\/[-a-z\d%_.~+]*)*(\?[;&a-z\d%_.~+=-]*)?(\#[-a-z\d_]*)?$/.test(url)){this.dataSource.environment.url=url;if($.isPlainObject(environment.params)){for(key in params){if(isScalar(environment.params[key])){this.dataSource.environment.params[key]=environment.params[key]+'';}}}}
...
if(instance.dataStore.item){this.setTaoVar(URI.ITEM,instance.dataStore.item);}}};if(this.dataSource.environment.type=='manual'&&source){populateData(this,source);}
...
function getSubjectLogin(){var subject=getSubject();return(subject)?subject[URI.SUBJETC_LOGIN]:false;}
function getSubjectName(){var subject=getSubject();return(subject)?subject[URI.SUBJETC_FIRSTNAME]+' '+subject[URI.SUBJETC_LASTNAME]:false;}
...
function getProcessExecution(){return taoStack.getTaoVar(URI.PROCESS);}
