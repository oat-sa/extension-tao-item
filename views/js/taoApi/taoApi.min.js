...
if($.isPlainObject(environment.params)){for(key in environment.params){if(isScalar(environment.params[key])&&!this.destinationService.params[key]){this.destinationService.params[key]=environment.params[key];}}}
...
function finish(){$(window).trigger(STATE.ITEM.FINISHED);}
...
$(window).bind(STATE.ITEM.FINISHED,push);var eventTracer=new EventTracer();function logEvent(elementName,eventType,data){eventTracer.feedTrace(elementName,eventType,new Date().getTime(),data);}
...
$(window).bind(STATE.ITEM.FINISHED,function(){eventTracer.sendAllFeedTrace_now();});