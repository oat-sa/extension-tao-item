...
return i;}};
var QTIWidget=QTIWidget||{};QTIWidget.slider=function(ctx){$(ctx.qti_item_id).append("<input type='hidden' id='"+ctx.qti_item_id.substring(1,ctx.qti_item_id.length)+"_qti_slider_value' />").append("<div class='qti_slider'></div>").append("<div class='qti_slider_label'></div>");var containerWidth=parseInt($(ctx.qti_item_id).width());var min=parseInt(ctx.opts['lowerBound']);var max=parseInt(ctx.opts['upperBound']);var step=parseInt(ctx.opts['step']);var stepLabel=(ctx.opts['stepLabel']===true||ctx.opts['stepLabel']==='true');var reverse=(ctx.opts['reverse']===true||ctx.opts['reverse']==='true');var orientation='horizontal';if($.inArray(ctx.opts['orientation'],['horizontal','vertical'])>-1){orientation=ctx.opts['orientation'];}
...
$(ctx.qti_item_id+' .qti_slider_label').append("<span class='slider_min'>"+displayMin+"</span>").append("<span class='slider_cur highlight'>"+displayMin+"</span>").append("<span class='slider_max'>"+displayMax+"</span>");}
else{$(ctx.qti_item_id).addClass('qti_slider_step');var stepSpacing=20;var displayRatio=1;if((max*20)>sliderSize){do{stepSpacing=(sliderSize/(max/displayRatio));displayRatio++;}while(stepSpacing<25);displayRatio--;}
...
i-=(step*displayRatio);if(i!=max){if(i<max){$(ctx.qti_item_id+' .qti_slider_label span:last').remove();}
...
$(ctx.qti_item_id+' .qti_slider_label').append($iStep);}}
var $sliderVal=$(ctx.qti_item_id+"_qti_slider_value");var val=min;if((reverse&&orientation=='horizontal')||(!reverse&&orientation=='vertical')){val=max;}
if(ctx.opts["values"]){if(typeof(ctx.opts["values"])=='string'&&ctx.opts["values"]!=''){val=ctx.opts["values"];}}
$(ctx.qti_item_id+' .qti_slider').slider({value:((reverse&&orientation=='horizontal')||(!reverse&&orientation=='vertical'))?max-val:val,min:min,max:max,step:step,orientation:orientation,slide:function(event,ui){var val=ui.value;if((reverse&&orientation=='horizontal')||(!reverse&&orientation=='vertical')){val=max-ui.value;}
$sliderVal.val(val);QTIWidget.slider_highlight(ctx,val);}});$sliderVal.val(val);QTIWidget.slider_highlight(ctx,val);};QTIWidget.slider_highlight=function(ctx,value){if(!$(ctx.qti_item_id).hasClass('qti_slider_step'))$(ctx.qti_item_id+' '+'.slider_cur').text(value);else{$(ctx.qti_item_id+' .qti_slider_label span').removeClass('highlight');$(ctx.qti_item_id+' .qti_slider_label span._'+value).addClass('highlight');}}
QTIWidget.upload=function(ctx){var uploaderElt=$(ctx.qti_item_id+'_uploader');if(uploaderElt.length>0){var fileExt='*';if(ctx.opts['ext']){if(ctx.opts['ext']!=''){fileExt='*.'+ctx.opts['ext'];}}
...
if(_this.opts["maxAssociations"]==1){result.value=elt;}else if(_this.opts["maxAssociations"]==0||result.value.length<_this.opts["maxAssociations"]){result.value.push(elt);}});return result;};this.text=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":null};if($("#"+_this.id).get(0).nodeName.toLowerCase()!='div'){switch(_this.opts['baseType']){case"integer":result.value=parseInt($("#"+_this.id).val());break;case"float":result.value=parseFloat($("#"+_this.id).val());break;case"string":default:result.value=$("#"+_this.id).val();}}
...
return result;};this.match=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":(_this.opts["maxAssociations"]==1)?null:[]};$("#"+_this.id+" .tabActive").each(function(){var subset=new Object();var classes=$(this).attr('class').split(' ');if(classes.length>0){var i=0;while(i<classes.length){if(/^xnode_/.test(classes[i])){subset[0]=classes[i].replace('xnode_','');}
if(/^ynode_/.test(classes[i])){subset[1]=classes[i].replace('ynode_','');}
i++;}
if(_this.opts["maxAssociations"]==1){result.value=subset;}else if(_this.opts["maxAssociations"]==0||result.value.length<_this.opts["maxAssociations"]){result.value.push(subset);}}});return result;};this.hotspot=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":[]};$("#"+_this.id+" li.activated").each(function(){if(_this.opts["maxChoices"]!=1)
...
return result;};this.graphic_associate=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":(_this.opts["maxAssociations"]==1)?null:[]};var pairs=$("#"+_this.id).data('pairs');for(i in pairs){var pair=pairs[i].split(' ');if(pair.length==2){if(_this.opts["maxAssociations"]==1){result.value=pair;}else if(_this.opts["maxAssociations"]==0||result.value.length<_this.opts["maxAssociations"]){result.value.push(pair);}}}
...
function qti_init_interaction(initObj){var myQTIWidgetFactory=new QTIWidgetFactory(initObj);var myResultCollector=new QTIResultCollector(initObj);var typeName=initObj["type"].replace('qti_','').replace('_interaction','');myQTIWidgetFactory.build(typeName);$("#qti_validate").bind("click",function(e){e.preventDefault();$('body').css('cursor','wait');var result=myResultCollector[typeName].apply();if(typeof(matchingSetResponses)=='function'){console.log('result',result);matchingSetResponses([result]);}});}
