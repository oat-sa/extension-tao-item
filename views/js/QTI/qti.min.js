...
if(opposite.length>0){var oppositeId=opposite.attr('id').replace('pair_','');if(_matchGroup.length>0){if($.inArray(oppositeId,_matchGroup)<0){return false;}}
var _oppositeMatchGroup=_this.opts["matchMaxes"][oppositeId]["matchGroup"];if(_oppositeMatchGroup.length>0){if($.inArray(draggedId,_oppositeMatchGroup)<0){return false;}}}
...
var _matchMax=Number(_this.opts["matchMaxes"][draggedId]["matchMax"]);var _current=Number(_this.opts["matchMaxes"][draggedId]["current"]);var _matchGroup=_this.opts["matchMaxes"][draggedId]["matchGroup"];if(_matchGroup.length>0){if($.inArray($(this).attr('id'),_matchGroup)<0){return false;}}
var _gapMatchGroup=_this.opts["matchMaxes"][$(this).attr('id')]["matchGroup"];if(_gapMatchGroup.length>0){if($.inArray(draggedId,_gapMatchGroup)<0){return false;}}
...
else{if(associations.length<maxAssociations||maxAssociations==0){var nodeXY=getNodeXY(elt);var _rowMatchGroup=_this.opts["matchMaxes"][nodeXY.xnode.id]['matchGroup'];if(_rowMatchGroup.length>0){if($.inArray(nodeXY.ynode.id,_rowMatchGroup)<0){return false;}}
var _colMatchGroup=_this.opts["matchMaxes"][nodeXY.ynode.id]['matchGroup'];if(_colMatchGroup.length>0){if($.inArray(nodeXY.xnode.id,_colMatchGroup)<0){return false;}}
...
result.value=this.id.replace(/^hottext_choice_/,'');});return result;};this.gap_match=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":(_this.opts["maxChoices"]!=1)?[]:null};$("#"+_this.id+" .filled_gap").each(function(){var firstId=$(this).attr('id').replace('gap_','');var lastId=$(this).parent().attr('id');result.value.push({0:firstId,1:lastId});});return result;};this.match=function(){var result={"identifier":_this.opts['responseIdentifier'],"value":[]};$("#"+_this.id+" .tabActive").each(function(){var subset=new Object();var classes=$(this).attr('class').split(' ');if(classes.length>0){var i=0;while(i<classes.length){if(/^xnode_/.test(classes[i])){subset[0]=classes[i].replace('xnode_','');}
...
myQTIWidget[typeName].apply();$("#qti_validate").bind("click",function(){var result=myResultCollector[typeName].apply();matchingSetResponses([result]);matchingEvaluate();});}