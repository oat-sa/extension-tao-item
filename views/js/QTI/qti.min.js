...
$(ctx.qti_item_id+' .qti_slider_label').append("<span class='slider_min'>"+displayMin+"</span>").append("<span class='slider_cur'>"+displayMin+"</span>").append("<span class='slider_max'>"+displayMax+"</span>");}
...
$(ctx.qti_item_id+' '+'.slider_cur').text(val);$(ctx.qti_item_id+' .qti_slider').slider({value:val,min:min,max:max,step:step,orientation:orientation,slide:function(event,ui){var val=ui.value;if((reverse&&orientation=='horizontal')||(!reverse&&orientation=='vertical')){val=max-ui.value;}
$sliderVal.val(val);$(ctx.qti_item_id+' '+'.slider_cur').text(val);}});$sliderVal.val(val);};QTIWidget.upload=function(ctx){var uploaderElt=$(ctx.qti_item_id+'_uploader');if(uploaderElt.length>0){var fileExt='*';if(ctx.opts['ext']){if(ctx.opts['ext']!=''){fileExt='*.'+ctx.opts['ext'];}}
