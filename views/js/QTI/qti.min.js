...
$(ctx.qti_item_id).data('pairs',pairs);$(this).unbind("mousedown");});var line=e.data.zeline;line.attr("stroke","red");$(this).unbind("mousedown");});if(ctx.opts["maxAssociations"]>0){countChoices--;$(ctx.qti_item_id+" .link_counter").text(countChoices);}
e.data.zis.toFront();}
...
new AsyncFileUpload(uploaderElt,{"scriptData":{'session_id':ctx.opts['session_id']},"basePath":ctx.wwwPath,"rootUrl":'',"fileDesc":'Allowed files type: '+fileExt,"fileExt":fileExt,"target":ctx.qti_item_id+'_data',"folder":"/"});}};
...
else{if(associations.length<maxAssociations||maxAssociations==0){var nodeXY=getNodeXY(jElement);var _rowMatchGroup=ctx.opts["matchMaxes"][nodeXY.xnode.id]['matchGroup'];if(_rowMatchGroup.length>0){if($.inArray(nodeXY.ynode.id,_rowMatchGroup)<0){$(this).effect("highlight",{color:'#B02222'},1000);return false;}}
var _colMatchGroup=ctx.opts["matchMaxes"][nodeXY.ynode.id]['matchGroup'];if(_colMatchGroup.length>0){if($.inArray(nodeXY.xnode.id,_colMatchGroup)<0){$(this).effect("highlight",{color:'#B02222'},1000);return false;}}
...
else if(rowMatch>1){var rowMatched=$(ctx.qti_item_id+" ."+nodeXY.xnode['class']+".tabActive").length;if(rowMatched>=rowMatch){$(this).effect("highlight",{color:'#B02222'},1000);return false;}}
...
else if(colMatch>1){var colMatched=$(ctx.qti_item_id+" ."+nodeXY.ynode['class']+".tabActive").length;if(colMatched>=colMatch){$(this).effect("highlight",{color:'#B02222'},1000);return false;}}
...
shapes[identifier].attr({"stroke-opacity":"0","stroke-width":"0"});if(ctx.graphicDebug){shapes[identifier].attr({"stroke-width":"3px","stroke-opacity":"1","stroke":"blue"});}
shapes[identifier].id=identifier;}}
var collisables=[];for(i in shapes){collisables.push(shapes[i]);}
$(ctx.qti_item_id+' .svg-container').droppable({drop:function(event,ui){var draggedId=$(ui.draggable).parent().attr("id");var offset=$(this).offset();var x=event.pageX-offset.left;var y=event.pageY-offset.top;var result=raphaelcollision(paper,collisables,x,y);if(result.length>0){fillGap($(this),$(ui.draggable),result[0][2].id,result[0][2]);}}});}
