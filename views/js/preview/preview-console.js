...
//FIXME this file should be completely removed, burned and thrown away.
define(['jquery'], function($) {
    ...
    return {
        ...
        setup: function() {
            ...
            var $previewConsole = $('#preview-console', window.parent.document);
            ...
            $('body').ajaxSuccess(function(event, request, settings) {
                ...
                if (/LegacyPreviewApi/.test(settings.url)) {
                    ...
                    if (/save$/.test(settings.url)) {
                        data = unserializeUrl(decodeURIComponent(settings.data));
                        for (key in data) {
                            if (key !== 'token') {
