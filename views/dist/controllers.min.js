...
var $container=injectButtonPlaceholder(uri);$container&&$container.length&&addCategorySwitch($container,uri,exposed)})}).catch(handleError)}}}),define("taoItems/runtime/ItemServiceImpl",["jquery"],function($){function ItemServiceImpl(data){this.itemId=data.itemId,this.serviceApi=data.serviceApi,this.resultApi=data.resultApi||null,this.params=data.params||{},this.responses={},this.scores={},this.events={},this.connected=!1;var rawstate=this.serviceApi.getState(),state=void 0===rawstate||null===rawstate?{}:$.parseJSON(rawstate);this.stateVariables="object"==typeof state?state:{},this.beforeFinishCallbacks=[]}
return ItemServiceImpl.prototype.connect=function(frame){!1===this.connected&&frame.contentWindow&&(frame.contentWindow.itemApi=this,"function"==typeof frame.contentWindow.onItemApiReady&&(frame.contentWindow.onItemApiReady(this),this.connected=!0))},ItemServiceImpl.prototype.saveResponses=function(valueArray){for(var attrname in valueArray)this.responses[attrname]=valueArray[attrname]},ItemServiceImpl.prototype.traceEvents=function(eventArray){for(var attrname in eventArray)this.events[attrname]=eventArray[attrname]},ItemServiceImpl.prototype.saveScores=function(valueArray){for(var attrname in valueArray)this.scores[attrname]=valueArray[attrname]},
ItemServiceImpl.prototype.getUserPropertyValues=function(property,callback){this.serviceApi.getUserPropertyValues(property,callback)},ItemServiceImpl.prototype.beforeFinish=function(callback){this.beforeFinishCallbacks.push(callback)},ItemServiceImpl.prototype.finish=function(){for(var self=this,i=0;i<this.beforeFinishCallbacks.length;i++)this.beforeFinishCallbacks[i]();this.submit(function(){self.serviceApi.finish()})},ItemServiceImpl.prototype.submit=function(after){var self=this;this.serviceApi.setState(JSON.stringify(self.stateVariables),function(){
...
for(var v in values)this.stateVariables[v]=values[v];this.serviceApi.setState(JSON.stringify(this.stateVariables))},ItemServiceImpl}),define("taoItems/preview/actionBarHook",["jquery","lodash","core/errorHandler","core/promise"],function($,_,errorHandler,Promise){"use strict";function isValidConfig(toolconfig){return!(!_.isObject(toolconfig)||!toolconfig.hook)}function triggerItemLoaded(tool){tool&&tool.itemLoaded&&tool.itemLoaded()}function initExtraButtons($toolContainer,id,toolconfig){return _.isString(toolconfig)&&(toolconfig={hook:toolconfig}),tools[id]=null,$toolContainers=$toolContainers.add($toolContainer),new Promise(function(resolve){
isValidConfig(toolconfig)?require([toolconfig.hook],function(hook){var $button,$existingBtn;isValidHook(hook)?(hook.init(id,toolconfig),$existingBtn=$toolContainer.children('[data-control="'+id+'"]'),$existingBtn.length&&(hook.clear($existingBtn),$existingBtn.remove()),tools[id]=hook,$button=hook.render(),_appendInOrder($toolContainer,$button),$button.trigger("ready"+_ns,[hook]),triggerItemLoaded(hook),resolve(hook)):(errorHandler.throw(_ns,"invalid hook format"),resolve(null))},function(e){errorHandler.throw(_ns,"the hook amd module cannot be found"),resolve(null)}):(errorHandler.throw(_ns,"invalid tool config format"),resolve(null))})}
function _appendInOrder($container,$button){var $after,$before,order=$button.data("order");"last"===order?$container.append($button):"first"===order?$container.prepend($button):(order=_.parseInt(order),_.isNaN(order)?$container.append($button):($container.children(".action").each(function(){var $btn=$(this),_order=$btn.data("order");if("last"===_order)$before=$btn,$after=null;else if("first"===_order)$before=null,$after=$btn;else{if(_order=_.parseInt(_order),_.isNaN(_order)||_order>order)return $before=$btn,$after=null,!1;_order===order?$after=$btn:_order<order&&($after=$btn,$before=null)}}),
$after?$after.after($button):$before?$before.before($button):$container.append($button)))}function isValidHook(hook){return _.isObject(hook)&&_(["init","render","clear","isVisible"]).reduce(function(result,method){return result&&_.isFunction(hook[method])},!0)}var _ns=".previewActionBarHook",tools={},$toolContainers=$();return $(document).off(".preview").on("itemunload.preview",function(){$toolContainers.each(function(){var $toolContainer=$(this);_.forEach(tools,function(tool,id){var $btn=$toolContainer.children('[data-control="'+id+'"]');$btn.length&&(tool.clear($btn),$btn.remove())})})}),{isValid:isValidConfig,initExtraButtons:initExtraButtons}}),
define("taoItems/controller/preview/itemRunner",["module","jquery","lodash","util/encode","serviceApi/ServiceApi","serviceApi/PseudoStorage","serviceApi/UserInfoService","taoItems/runtime/ItemServiceImpl","taoItems/preview/actionBarHook","urlParser"],function(module,$,_,encoder,ServiceApi,PseudoStorage,UserInfoService,ItemServiceImpl,actionBarHook,UrlParser){"use strict";var _initCustomButtons=function(){var config=module.config(),buttons=config.extraButtons||{},$container=$(".extra-button-action-bar");_.forIn(buttons,function(config,id){actionBarHook.initExtraButtons($container,id,config)})};return{start:function(options){
var conf=_.merge(module.config(),options||{});if(conf.previewUrl){var resultServer=_.defaults(conf.resultServer,{module:"taoResultServer/ResultServerApi",endpoint:"",params:{}});require([resultServer.module],function(ResultServerApi){var resultServerApi=new ResultServerApi(resultServer.endpoint,resultServer.params),serviceApi=new ServiceApi(conf.previewUrl,{},"preview",new PseudoStorage,new UserInfoService(conf.userInfoServiceRequestUrl,{})),itemApi=new ItemServiceImpl({serviceApi:serviceApi,resultApi:resultServerApi}),callUrl=new UrlParser(serviceApi.getCallUrl()),isCORSAllowed=callUrl.checkCORS();callUrl.addParam("clientConfigUrl",conf.clientConfigUrl)
;var state,$frame=$('<iframe id="preview-container" name="preview-container" src="'+callUrl.getUrl()+'"></iframe>');try{state=JSON.parse(encoder.decodeBase64(conf.state))}catch(e){state=null}state&&itemApi.setVariables(state),$frame.on("load",function(){var frame=this;itemApi.connect(frame),!0===isCORSAllowed&&(frame.contentWindow.__knownParent__=!0),$(document).off("itemready").on("itemready",function(){itemApi.connect(frame),frame.contentWindow.__knownParent__&&(frame.contentWindow.document.body.className+=" tao-preview-scope",_initCustomButtons())})}),$(".preview-item-container").append($frame),$("#finishButton").click(function(){itemApi.finish()})})}}}
}),define("taoItems/controller/routes",[],function(){"use strict";return{Items:{deps:"controller/items/action",actions:{editItem:"controller/items/edit",editItemClass:"controller/items/editItemClass"}},ItemPreview:{actions:{index:"controller/preview/itemRunner"}}}}),define("taoItems/controller/runtime/itemRunner",["jquery","lodash","iframeNotifier","urlParser"],function($,_,iframeNotifier,UrlParser){"use strict";return{start:function(options){var $frame=$('<iframe id="item-container" class="toolframe" frameborder="0" style="width:100%;" scrolling="no"></iframe>');$frame.appendTo("body")
;var itemId=options.itemId,itemPath=options.itemPath,resultServer=_.defaults(options.resultServer,{module:"taoResultServer/ResultServerApi",params:{}}),itemService=_.defaults(options.itemService,{module:"taoItems/runtime/ItemServiceImpl",params:{}}),clientConfigUrl=options.clientConfigUrl,timeout=options.timeout||30;require([itemService.module,resultServer.module],function(ItemService,ResultServerApi){var resultServerApi=new ResultServerApi(resultServer.endpoint,resultServer.params),setIframeHeight=function(newHeight){var neverResized=150===$frame.height(),resize=function(newHeight,notifyParent){$frame.css({height:newHeight}),
notifyParent&&iframeNotifier.parent("heightchange",newHeight)};resize(newHeight||$frame.contents().outerHeight(!0),!newHeight),neverResized&&_.delay(function(){resize($frame.contents().outerHeight(!0),!0)},200)};window.onServiceApiReady=function(serviceApi){var itemApi=new ItemService(_.merge({serviceApi:serviceApi,itemId:itemId,resultApi:resultServerApi},{params:itemService.params})),itemUrl=new UrlParser(itemPath),isCORSAllowed=itemUrl.checkCORS();itemUrl.addParam("clientConfigUrl",clientConfigUrl),itemUrl.addParam("timeout",timeout),$(document).on("itemloaded",function(){setIframeHeight(),iframeNotifier.parent("serviceloaded"),
_.delay(setIframeHeight,1600),$frame.contents().find("img").on("load",_.throttle(function(){setIframeHeight()},50))}).on("responsechange",function(e,responseId,response){iframeNotifier.parent("responsechange",[responseId,response])}).on("stateready",function(e,elmentId,state){iframeNotifier.parent("stateready",[elmentId,state])}).on("itemready",function(){itemApi.connect($frame[0])}).on("itemcontentchange imageloaded",function(){setIframeHeight()}),$frame.on("load",function(){itemApi.connect($frame[0]),!0===isCORSAllowed&&(this.contentWindow.__knownParent__=!0),$("body").on("setheight",function(event,height){setIframeHeight(height)})}),
$frame.attr("src",itemUrl.getUrl())},_.defer(function(){iframeNotifier.parent("serviceready")})})}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))
